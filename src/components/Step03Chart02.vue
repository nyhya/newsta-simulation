...
export default {
  ...
  mounted() {
    this.myChart = window.echarts.init(
      document.getElementById("Step03Chart02"),
      null,
  ...
  methods: {
    setData() {
      var data01 = this.$store.state.calData.resultData.B155.slice(0, 12).map(
        function (d) {
          return parseInt(d / 1000);
        }
      );
      var data02 = this.$store.state.calData.resultData.B157.slice(0, 12).map(
        function (d) {
          return parseInt(d / 1000);
        }
      );
      var data03 = this.$store.state.calData.resultData.B158.slice(0, 12).map(
        function (d) {
          return parseInt(d / 1000);
        }
      );
